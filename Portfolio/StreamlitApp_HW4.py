...
MODEL_INFO = {
    ...
    "inputs": [
        {"name": PARTNER_TICKER, "type": "number", "min": 0.0, "max": 10000.0, "default": 0.0, "step": 10.0},
        {"name": TARGET_TICKER, "type": "number", "min": 0.0, "max": 10000.0, "default": 0.0, "step": 10.0},
    ]
}
...
def display_explanation(input_df, session, aws_bucket):
    ...
    try:
        ...
        st.info("**Business Insight:** See the SHAP waterfall plot above for feature contributions.")

# Streamlit UI
st.set_page_config(page_title="Pairs Trading - ML Deployment", layout="wide")
st.title("📈 Pairs Trading Signal Predictor")
st.markdown(f"**Trading Pair:** {TARGET_TICKER} & {PARTNER_TICKER}")
...
if submitted:
    ...
    if status == 200:
        signal_int = int(res)
        signal_label = SIGNAL_MAP.get(signal_int, f"Unknown ({res})")
        st.metric("Prediction Result", signal_label)
